...
<script>
    // ПОЛНОЕ УНИЧТОЖЕНИЕ НАЗВАНИЯ СТРАНИЦЫ
    (function() {
        function killAllTitles() {
            // Удаляем все теги title
            var titles = document.querySelectorAll('title');
            for(var i = 0; i < titles.length; i++) {
                titles[i].remove();
            }
            // Очищаем document.title
            if(document.title !== '') {
                document.title = '';
            }
            // Удаляем мета-теги с названием
            var metaTags = document.querySelectorAll('meta[property="og:title"], meta[name="title"], meta[property="twitter:title"], meta[name="twitter:title"]');
            for(var i = 0; i < metaTags.length; i++) {
                metaTags[i].remove();
            }
        }
        
        // Блокируем возможность изменения title
        Object.defineProperty(document, 'title', {
            get: function() { return ''; },
        ...
        // Запускаем каждые 100мс (на случай если хостинг добавляет позже)
        setInterval(killAllTitles, 100);
        
        // Отслеживаем новые добавленные элементы
        var observer = new MutationObserver(function() {
            var titles = document.querySelectorAll('title');
            if(titles.length > 0 || document.title !== '') {
                killAllTitles();
            }
        });
        ...
        window.addEventListener('load', killAllTitles);
    })();
</script>
